...
def k_path(kpts,nk,model=None,report=True):
    r"""
      ...
      :param kpts: Array of k-vectors in reciprocal space between which
        interpolated path should be constructed. These k-vectors must
        be given in reduced coordinates.  As a special case, in
        1D k-space kpts may be a string:

        * "full"  -- Implies  [ 0.0, 1.0]  (full BZ)
        * "fullc" -- Implies  [-0.5, 0.5]  (full BZ, centered)
        * "half"  -- Implies  [ 0.0, 0.5]  (half BZ)

      :param nk: 
        If model is None:
	   Number of k-point intervals per segment
        Else:
           Total number of k-points to be used in making the plot.

      :param model: Optional parameter specifying PythTB model,
        which is used for access to lattice vectors so that
        k-space distances can be calculated (default is None).
      ...
      :returns:
        If model is None:
          * **kipts**     -- Array of interpolated k-points
        Else:
          * **kipts**     -- Array of interpolated k-points
          * **kd_points** -- Array giving accum. k-distance to each k-point
          * **kd_nodes**  -- Array giving accum. k-distance to each node
      ...
      Example usage::
        ...
        (kipts,kd_points,kd_nodes) = k_path(path,401,my_model)
        # solve for eigenvalues on that path
        evals = tb.solve_all(kipts)
        # then use evals, kd_points, and kd_nodes to plot bandstructure
        # (see examples)
    ...
    # now kpts should be an array
    k_list=np.array(kpts)

    if model is None:

        # The function does not have access to k-distances
        # For backward compatibility, proceed as in PythTB 1.6.2 
    ...
    # Since model is provided, compute scaling of k-space distances

    if report==True:
      print '----- begin k_path ----------'

    k_shape=k_list.shape
    if len(k_shape)==2:
        if k_shape[1]==1:
             # this is really a 1D k-space packaged in a rank-2 array
             # repackage is a simple rank-1 array
             k_list=k_list.flatten
             k_shape=k_list.shape
      
    if len(k_shape)==1:
        # this means k-space is 1D

        # there should be only starting and ending k-values
        kd_nodes=k_list
        if len(kd_nodes)!=2:
            raise Exception("\n\nFor 1D kspace, kpts must contain two values")

        # construct list of interpolated k-points
        kipts=np.zeros(nk,dtype=float)
        dk=(kd_nodes[1]-kd_nodes[0])/float(nk-1)
        for j in range(nk):
            kipts[j]=kd_nodes[0]+dk*float(j)

        # distances from starting point:
        kd_points=kipts-kipts[0]

        # we are done
        if report==True:
          print 'kd_nodes:', kd_nodes
          print '----- end k_path ----------'
        return(kipts,kd_points,kd_nodes)

    # remaining part is for case that k-space is of dimension 2 or greater

    n_nodes=len(kpts)
    k_dim=len(kpts[0])

    # initialize array kd_nodes (accumulated distances to nodes)
    kd_nodes=np.zeros(n_nodes,dtype=float)
    # compute kd_nodes
    ...
    # check whether k-space dimensions are consistent
    if k_list.shape[1]!=lat.shape[0]:
        print 'input k-space dimension is',k_list.shape[1]
        print 'k-space dimension taken from model is',lat.shape[0]
        raise Exception("\n\nk-space dimensions do not match")

    if report==True:
      print 'repeating real-space lattice vectors\n', lat

    # k_space metric is inverse of lat.lat-transpose
    k_metric = np.linalg.inv(np.dot(lat,lat.T))
    if report==True:
      print 'k-space metric tensor\n', k_metric
      print 'k-space coordinates of nodes\n', kpts

    # Find distances between nodes and set kd_nodes, which is
    ...
    for n in range(1,n_nodes):
        ...
        if report==True:
          print 'segment',n,dk,'  distance =',dklen
        kd_nodes[n]=kd_nodes[n-1]+dklen
    ...
    for n in range(1,n_nodes):
        ...
        for j in range(n_i,n_f+1):
            ...
            kd_points[j]=kd_i+frac*(kd_f-kd_i)
            kipts[j]=k_i+frac*(k_f-k_i)
