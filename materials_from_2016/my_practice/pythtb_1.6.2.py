...
""")
    ...
    def _val_to_block(self,val):
        ...
        # spinfull case
            ...
            if use_val.shape==():
                ret[0,0]+=use_val             # since ret is a zeros matrix I think we could easily have set ret[0,0] = use_val instead of +=
                ret[1,1]+=use_val
            # if four numbers are given
            elif use_val.shape==(4,):         #only tuple object have .shape features and it comes in a tuple hence (4,)
                # diagonal
                ret[0,0]+=use_val[0]
                ret[1,1]+=use_val[0]
                # sigma_x
...
matrix.""")            
    ...
    def _gen_ham(self,k_input=None):
        ...
        if self._nspin==1:
            ...
            ham=np.zeros((self._norb,2,self._norb,2),dtype=complex)  # what's the difference between zeros(4,4) and zeros(2,2,2,2) why we used later here? 
