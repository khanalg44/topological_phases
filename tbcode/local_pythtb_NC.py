...
matrix.""")            
    ...
    # for single orbital case spins considered
    def _gen_ham2spin(self,k_input, t=1.):
        kpnt=2*np.pi*np.array(k_input)

        dim_ham = self._nspin * self._norb
        ham=np.zeros((dim_ham, dim_ham), dtype=complex)
   
        for i in range(dim_ham):
            ham[i,i] = -2*t*(np.cos(kpnt[0])+np.cos(kpnt[1]))
    ...
    def solve_all(self,k_list=None,eig_vectors=False):
        ...
        if not (k_list is None):
            ...
            print ('zero dimentional case, not implemented')
